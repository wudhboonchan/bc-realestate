        ...
        try {
          ...
          if (!res.ok) {
            ...
            const textInvoice = isBurmese
              ? `📄 ${propertyName}\nလစဉ် ဘေလ်စာရင်း (${bill.monthYear})\nအခန်း: ${bill.roomNumber}\nအိမ်ငှား: ${bill.tenantName}\n\n• အခန်းခ: ฿${(bill.rentAmount || 0).toLocaleString()}\n• ရေဖိုး (${bill.waterUnits || 0} ယူနစ်): ฿${(bill.waterAmount || 0).toLocaleString()}\n• မီးဖိုး (${bill.elecUnits || 0} ယူနစ်): ฿${(bill.elecAmount || 0).toLocaleString()}\n• အမှိုက်သိမ်းခ: ฿${(bill.garbageFee || 0).toLocaleString()}\n\n💰 စုစုပေါင်း ပေးရန်: ฿${(bill.totalAmount || 0).toLocaleString()}\n⏰ ပေးရန်ရက်: ${bill.dueDate}\n\n🔗 ဘေလ်ကြည့်ရန် / ငွေပေးချေရန်:\n${baseUrl}/invoice/${bill.id}`
